(function(){
	...
	routehelper.$inject = ['$rootScope','$state','routehelperConfig','$cookieStore','$location'];
	...
	function routehelper($rootScope,$state,routehelperConfig,$cookieStore,$location){
		var routes = [];
		...
		function configureRoutes(routes){
			...
			$rootScope.$on('$locationChangeStart', function(event,next, fromUrl) {
				if($cookieStore.get("estaConectado")==false || $cookieStore.get("estaConectado")==undefined){
		    		if(next=='http://localhost:3000/#/perfil' ||next=='http://localhost:3000/#/libros' || next=='http://localhost:3000/#/reservas'){
		    			$location.path('/user');
		    		}
		    	}else{
		    		if(next=='http://localhost:3000/#/user'){
